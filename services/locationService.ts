...
export const ACCURACY_THRESHOLD = 50; // High precision limit (meters)

const isValidCoord = (num: any) => typeof num === 'number' && !isNaN(num);

/**
 * Robust Browser Location Fetcher (Single Shot)
 * Utilizes hardware GPS with aggressive timeout to ensure fresh readings.
 */
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        switch(err.code) {
            ...
            case 2: msg = "Position unavailable (low signal)."; break;
...
export const watchLocation = (
  ...
  return navigator.geolocation.watchPosition(
    ...
    (err) => {
      if (err.code !== 3) console.warn("Location Watch Error:", err);
      onError(err);
    },
    { 
      enableHighAccuracy: true, 
